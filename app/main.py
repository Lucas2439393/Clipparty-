"""
Lokale HTTP API voor de toekomstige Lovable-GUI. Draait volledig lokaal op de
Windows-pc van de gebruiker (uvicorn app.main:app --host 127.0.0.1 --port 8756).

Endpoints, exact zoals gespecificeerd:
...
# CORS open voor lokale GUI-ontwikkeling (Lovable draait typisch op een ander poort/origin).
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def health():
    """
    Naast de algemene status/warnings ook expliciet welk Groq-model
    daadwerkelijk gebruikt zou worden -- automatisch bepaald, nooit
    hardcoded aangenomen. Dit endpoint triggert zelf de model-resolutie
    (met een korte cache), zodat een "model_not_found" al zichtbaar is
    vóórdat er een echte job gestart wordt.
    """
...
def upload_file(file: UploadFile = File(...)):
    """Ontvangt een bestand vanuit de drag&drop-UI (briefing of bronvideo) en slaat
    het lokaal op zodat het pad meegegeven kan worden aan /api/campaign/start."""
    safe_name = f"{uuid.uuid4().hex[:8]}_{Path(file.filename).name}"
    dest = UPLOADS_DIR / safe_name
    with dest.open("wb") as out:
        shutil.copyfileobj(file.file, out)

    # Directe upload-validatie: te lange video's worden geweigerd vóór een job
    # en vóór iedere Groq/Whisper-call.
...
def connect_account(req: ConnectionRequest):
    ...
    # This records a local connection only. Real OAuth/API access is only
    # marked connected after the platform's own authorization flow succeeds.
